var GeoJSON = (function() {
  ...
  function getGeometries(geometry) {
    var i, il, polygon, geometries = [], sub;
    ...
    var j, jl, p, lat = 1, lon = 0, outer = [], inner = [];
    p = polygon[0];
  ...
  function pixelToGeo(x, y) {
  	var RAD = 180/Math.PI,HALF_PI = Math.PI/2,MapSize = 134217728;
    var res = {};
    x /= MapSize;
    y /= MapSize;
    res["lat"] = y <= 0  ? 90 : y >= 1 ? -90 : RAD * (2 * Math.atan(Math.exp(Math.PI * (1 - 2*y))) - HALF_PI),
    res["lng"] = (x === 1 ?  1 : (x%1 + 1) % 1) * 360 - 180;
    return res;
}

function geoToPixel(lat, lon) {
	  var PI=Math.PI, HALF_PI = PI/2,QUARTER_PI = PI/4,MapSize = 134217728;
    var lat  = Math.min(1, Math.max(0, 0.5 - (Math.log(Math.tan(QUARTER_PI + HALF_PI * lat / 180)) / PI) / 2)),
        lng = lon/360 + 0.5;
    return {
        x: lng * MapSize <<0,
        y: lat * MapSize <<0
